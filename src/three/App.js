...
function updateGUI(viewer) {
  ...
  slice.add(viewer.params.slice, "x", 0, 1, 0.01).listen().onChange(viewer.render);
  slice.add(viewer.params.slice, "y", 0, 1, 0.01).listen().onChange(viewer.render);
  slice.add(viewer.params.slice, "z", 0, 1, 0.01).listen().onChange(viewer.render);

  const sketch = gui.addFolder("sketch").close();
  sketch.add(viewer.params, "dot", 1, 50, 1).name("dot").onChange(viewer.render);
  sketch.add(viewer.params, "depth", 0, 20, 1).name("depth").onChange(viewer.render);
  sketch.add(viewer.params, "erase").name("erase").onChange(viewer.render);

  const threshold = gui.addFolder("threshold").close();
  threshold.add(viewer.params, "min", 0, 1, 0.01).name("min").onChange(viewer.render);
  threshold.add(viewer.params, "max", 0, 1, 0.01).name("max").onChange(viewer.render);
}
